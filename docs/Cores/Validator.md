...
`Validator` คือคลาสสำหรับตรวจสอบข้อมูล (Data Validation) ภายในระบบ Framework ออกแบบให้ใช้งานง่าย รองรับ:

- หลาย rule ต่อ 1 field
- ข้อความผิดพลาดแบบกำหนดเอง (custom message)
- กำหนด label สำหรับแสดงผล error
- Rule แบบมี parameter เช่น `min:3`, `between:1,10`
- Rule ที่เชื่อมกับฐานข้อมูล เช่น `unique`, `exists`
...
ระบบจะ cache ผลลัพธ์ไว้ — เรียก `passes()` หรือ `fails()` ซ้ำจะไม่ validate ใหม่
...
- `min` — `'age' => 'min:18'`
- `max` — `'username' => 'max:20'`
- `between` — `'age' => 'between:18,60'`
- `match` (ยืนยันค่าเหมือน field อื่น) — `'password_confirmation' => 'match:password'`
- `unique` (เช็คค่าซ้ำในฐานข้อมูล) — `'email' => 'unique:users,email'` (รูปแบบ: `unique:table,column`) — ต้องมี Database instance ส่งเข้า Validator
- `exists` (ตรวจสอบว่ามีข้อมูลอยู่จริง) — `'user_id' => 'exists:users,id'` (รูปแบบ: `exists:table,column`)
...
- ค่า "0" จะไม่ถูกมองว่าเป็น empty
